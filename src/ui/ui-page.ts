...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;font-family:'Manrope',sans-serif;color:var(--text);background:
    ...
    .tableWrap{max-height:560px;overflow:auto;border:1px solid rgba(87,70,50,.08);border-radius:18px;background:rgba(255,255,255,.72)}
    table{width:100%;border-collapse:collapse;min-width:880px}th,td{padding:14px 12px;text-align:left;border-bottom:1px solid rgba(48,84,64,.1);vertical-align:top}th{position:sticky;top:0;background:#eff7f1;font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:var(--muted);z-index:1}
    tbody tr{cursor:pointer;transition:.18s background-color}.jobRow:hover,.jobRow.active{background:rgba(47,143,91,.08)}
...
function renderJobs(){el('jobCount').textContent=state.jobs.length+' jobs';if(!state.jobs.length){el('jobList').innerHTML='<div class="empty">No jobs yet.</div>';return;}el('jobList').innerHTML='<div class="tableWrap"><table><thead><tr><th>Subject</th><th>Recipient</th><th>Status</th><th>Attempts</th><th>Created</th><th>Next Run</th><th>Last Error</th></tr></thead><tbody>'+state.jobs.map(j=>'<tr class="jobRow'+(state.selectedJobId===j.id?' active':'')+'" data-id="'+j.id+'"><td><strong>'+esc(j.payload.subject)+'</strong></td><td>'+esc(j.payload.to)+'</td><td>'+badge(j.status)+'</td><td>'+esc(j.attemptsMade)+' / '+esc(j.maxAttempts)+'</td><td>'+esc(fmt(j.createdAt))+'</td><td>'+esc(fmt(j.nextRunAt))+'</td><td>'+(j.lastError?'<span class="mono">'+esc(j.lastError)+'</span>':'<span class="meta">None</span>')+'</td></tr>').join('')+'</tbody></table></div>';document.querySelectorAll('#jobList [data-id]').forEach(n=>n.addEventListener('click',()=>{state.selectedJobId=n.getAttribute('data-id');renderJobs();renderSelected();}));}
...
function renderSelected(){const j=state.jobs.find(x=>x.id===state.selectedJobId)||(state.jobs[0]||null);if(!j){el('jobDetail').innerHTML='Select a job to inspect details.';el('timeline').innerHTML='<div class="empty">Timeline will appear here.</div>';el('jobLogs').innerHTML='<div class="empty">Related logs will appear here.</div>';return;}if(!state.selectedJobId)state.selectedJobId=j.id;const failure=j.failureDetails?'<div class="detailBlock"><h3 style="margin-bottom:10px;">Failure Details</h3><div class="metaGrid"><div class="labelMini"><span>Message</span><strong>'+esc(j.failureDetails.message)+'</strong></div><div class="labelMini"><span>Error Name</span><strong>'+esc(j.failureDetails.name)+'</strong></div><div class="labelMini"><span>Code</span><strong>'+esc(j.failureDetails.code||'N/A')+'</strong></div><div class="labelMini"><span>SMTP Command</span><strong>'+esc(j.failureDetails.command||'N/A')+'</strong></div><div class="labelMini"><span>SMTP Response</span><strong>'+esc(j.failureDetails.response||'N/A')+'</strong></div><div class="labelMini"><span>Response Code</span><strong>'+esc(j.failureDetails.responseCode??'N/A')+'</strong></div><div class="labelMini"><span>Last Error</span><strong>'+esc(j.lastError||'N/A')+'</strong></div><div class="labelMini"><span>SMTP Username</span><strong>'+esc(j.payload.smtp.username||'None')+'</strong></div></div>'+(j.failureDetails.stack?'<div class="bodyBox mono" style="margin-top:12px;">'+esc(j.failureDetails.stack)+'</div>':'')+'</div>':'';el('jobDetail').innerHTML='<article class="item active" style="cursor:default"><div class="itemTop"><div><h3>'+esc(j.payload.subject)+'</h3><small>'+esc(j.payload.to)+'</small></div>'+badge(j.status)+'</div><div class="mini"><div class="labelMini"><span>Job ID</span><strong class="mono">'+esc(j.id)+'</strong></div><div class="labelMini"><span>SMTP Host</span><strong>'+esc(j.payload.smtp.host)+'</strong></div><div class="labelMini"><span>Sender</span><strong>'+esc(j.payload.smtp.fromEmail)+'</strong></div><div class="labelMini"><span>Updated</span><strong>'+esc(fmt(j.updatedAt))+'</strong></div></div><div class="bodyBox">'+esc(j.payload.body)+'</div>'+(j.result?'<div class="detailBlock"><h3 style="margin-bottom:10px;">Delivery Result</h3><div class="metaGrid"><div class="labelMini"><span>Message ID</span><strong class="mono">'+esc(j.result.providerMessageId)+'</strong></div><div class="labelMini"><span>Accepted</span><strong>'+esc((j.result.accepted||[]).join(', ')||'None')+'</strong></div><div class="labelMini"><span>Rejected</span><strong>'+esc((j.result.rejected||[]).join(', ')||'None')+'</strong></div><div class="labelMini"><span>Response</span><strong>'+esc(j.result.response)+'</strong></div></div></div>':'')+failure+'</article>';el('timeline').innerHTML=j.history?.length?j.history.slice().reverse().map(h=>'<article class="timelineItem"><div class="itemTop"><div><strong>'+esc(h.event)+'</strong><small>'+esc(fmt(h.timestamp))+'</small></div>'+badge(h.status)+'</div><p class="meta" style="margin:10px 0 0">'+esc(h.message)+'</p></article>').join(''):'<div class="empty">No timeline entries yet.</div>';const rel=state.logs.filter(l=>l.jobId===j.id);el('jobLogs').innerHTML=rel.length?rel.map(renderLog).join(''):'<div class="empty">No logs found for this job yet.</div>';}
